    ...
    <div class="p-4">
        ...
        <nav>
            ...
            <?php $__currentLoopData = $modules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $module): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <?php elseif($key === 'kegiatan' && auth()->user()->hasRole('jamaah')): ?>
                    
                    <div x-data="{ open: <?php echo e(request()->routeIs('jamaah.kegiatan.*') || request()->routeIs('jamaah.pengumuman.*') || request()->routeIs('jamaah.sertifikat.*') ? 'true' : 'false'); ?> }">
                        <button @click="open = !open" 
                                class="flex items-center justify-between w-full px-4 py-3 text-gray-700 hover:bg-green-50 hover:text-green-700 rounded transition <?php echo e(request()->routeIs('jamaah.kegiatan.*') || request()->routeIs('jamaah.pengumuman.*') || request()->routeIs('jamaah.sertifikat.*') ? 'bg-green-100 text-green-700' : ''); ?>">
                        ...
                        <div x-show="open" x-collapse class="ml-6 mt-1 space-y-1">
                            <a href="<?php echo e(route('jamaah.pengumuman.index')); ?>" 
                               class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 rounded transition <?php echo e(request()->routeIs('jamaah.pengumuman.*') ? 'bg-green-100 text-green-700 font-semibold' : ''); ?>">
                                <i class="fas fa-bullhorn w-5 mr-2"></i>
                                <span>Pengumuman</span>
                            </a>
                            
                            <a href="<?php echo e(route('jamaah.kegiatan.index')); ?>" 
                               class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 rounded transition <?php echo e(request()->routeIs('jamaah.kegiatan.*') ? 'bg-green-100 text-green-700 font-semibold' : ''); ?>">
                                <i class="fas fa-calendar-check w-5 mr-2"></i>
                                <span>Kegiatan</span>
                            </a>
                            
                            <a href="<?php echo e(route('jamaah.sertifikat.index')); ?>" 
                               class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 rounded transition <?php echo e(request()->routeIs('jamaah.sertifikat.*') ? 'bg-green-100 text-green-700 font-semibold' : ''); ?>">
...
<?php /**PATH E:\Ney\Kuliah\Semester 7\MANAJEMEN PROYEK\manajemen_masjid\resources\views/layouts/sidebar.blade.php ENDPATH**/ ?>