...
<?php $__env->startSection('content'); ?>
    ...
    <script>
        ...
        function toggleInputMethod() {
            ...
            // Load peserta if from_peserta selected
            if (fromPeserta && selectedKegiatan) {
                loadPesertaFromKegiatan(selectedKegiatan);
            }
            
            updatePreview();
        }
        
        // Listen kegiatan selection change
        document.querySelector('select[name="kegiatan_id"]').addEventListener('change', function(e) {
            selectedKegiatan = e.target.value;
            const selectedOption = e.target.options[e.target.selectedIndex];
            
            if (selectedKegiatan) {
                // Check if "from_peserta" is selected
                const fromPeserta = document.querySelector('input[name="input_method"][value="from_peserta"]').checked;
                if (fromPeserta) {
                    loadPesertaFromKegiatan(selectedKegiatan);
                }
        ...
        // Listen template change
        document.querySelector('select[name="template"]').addEventListener('change', function(e) {
            selectedTemplate = e.target.value;
            updatePreview();
        });
        
        // Load participants from kegiatan
        function loadPesertaFromKegiatan(kegiatanId) {
            const infoDiv = document.getElementById('from-peserta-input');
            infoDiv.innerHTML = '<p class="text-sm text-gray-600"><i class="fas fa-spinner fa-spin mr-2"></i>Memuat data peserta...</p>';
            
            fetch(`/kegiatan/sertifikat/peserta?kegiatan_id=${kegiatanId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.length > 0) {
                        let checkboxes = data.map((p, index) => 
                            `<label class="flex items-center p-2 hover:bg-blue-50 rounded cursor-pointer">
                                <input type="checkbox" name="peserta_selected[]" value="${p.nama}" 
                                       class="mr-3 w-4 h-4 text-green-600 peserta-checkbox" checked>
                                <div class="flex-1">
                                    <span class="font-medium text-gray-800">${p.nama}</span>
                                    ${p.email ? `<span class="text-xs text-gray-500 ml-2">(${p.email})</span>` : ''}
                                </div>
                            </label>`
                        ).join('');
                            ...
                            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                <div class="flex items-center justify-between mb-3">
                                    <p class="text-sm text-blue-800 font-semibold">
                                        ...
                                        Pilih Peserta (${data.length} terdaftar)
                                    </p>
                                    <div class="flex gap-2">
                                        <button type="button" onclick="selectAllPeserta(true)" 
                                                class="text-xs px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700">
                                            <i class="fas fa-check-double mr-1"></i>Pilih Semua
                                        </button>
                                        <button type="button" onclick="selectAllPeserta(false)" 
                                                class="text-xs px-3 py-1 bg-gray-400 text-white rounded hover:bg-gray-500">
                                            <i class="fas fa-times mr-1"></i>Batal Pilih
                                        </button>
                                    </div>
                                </div>
                                <div class="max-h-60 overflow-y-auto space-y-1 bg-white rounded p-2">
                                    ${checkboxes}
                                </div>
                                <p class="text-xs text-blue-600 mt-2">
                                    <i class="fas fa-info-circle mr-1"></i>
                                    <span id="selected-count">${data.length}</span> peserta dipilih
                                </p>
                            </div>
                        `;
                        ...
                        infoDiv.innerHTML = `
                            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                <p class="text-sm text-yellow-800">
                                    ...
                                    Tidak ada peserta yang terdaftar pada kegiatan ini.
                                </p>
                            </div>
                        `;
                    }
        ...
        function selectAllPeserta(checked) {
            document.querySelectorAll('.peserta-checkbox').forEach(cb => {
                cb.checked = checked;
            });
            updateSelectedCount();
        ...
        function updatePreview() {
            const previewDiv = document.querySelector('.border-dashed').parentElement;
            ...
            const namaKegiatan = selectedOption.text || 'Nama Kegiatan';
            ...
            const templateColors = {
                'kajian': 'from-green-500 to-green-700',
                'workshop': 'from-blue-500 to-blue-700',
                'pelatihan': 'from-amber-600 to-amber-800',
                'default': 'from-gray-600 to-gray-800'
            };
            
            const color = templateColors[selectedTemplate] || templateColors.default;
            
            previewDiv.innerHTML = `
                <h3 class="text-lg font-semibold text-gray-800 mb-4">Preview Sertifikat</h3>
                <div class="border-2 border-gray-300 rounded-lg p-6 bg-white shadow-lg">
                    <div class="bg-gradient-to-r ${color} p-8 rounded-lg text-white text-center">
                        <div class="border-4 border-white border-double p-6">
                            <i class="fas fa-mosque text-4xl mb-4 opacity-75"></i>
                            <h2 class="text-3xl font-bold mb-2">SERTIFIKAT</h2>
                            <p class="text-lg mb-6 opacity-90">Diberikan Kepada</p>
                            
                            <div class="bg-white text-gray-800 px-6 py-3 rounded my-4">
                                <p class="text-2xl font-bold">NAMA PESERTA</p>
                            </div>
                            
                            <p class="text-sm mb-2">Telah mengikuti kegiatan</p>
                            <p class="text-xl font-semibold mb-4">${namaKegiatan.split('-')[0].trim()}</p>
                            
                            <div class="mt-6 pt-4 border-t border-white opacity-75">
                                <p class="text-xs">Template: ${selectedTemplate.toUpperCase()}</p>
                            </div>
                        </div>
                    </div>
                    <p class="text-xs text-center text-gray-500 mt-2">*Preview ini hanya simulasi, hasil akhir akan berbeda</p>
                </div>
            `;
        }

        // Show history tab if there's tab parameter in URL
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\Ney\Kuliah\Semester 7\MANAJEMEN PROYEK\manajemen_masjid\resources\views/modules/kegiatan/sertifikat/index.blade.php ENDPATH**/ ?>