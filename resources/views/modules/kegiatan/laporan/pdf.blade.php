...
<head>
    ...
    <title>Laporan Kegiatan - {{ $laporan->nama_kegiatan }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="header">
        ...
        <p>{{ $laporan->tanggal_pelaksanaan->format('d F Y') }}</p>
    </div>

    <!-- Status Badges -->
    <div style="margin-bottom: 20px;">
        <span class="badge badge-{{ $laporan->status }}">{{ strtoupper($laporan->status) }}</span>
        <span class="badge badge-{{ $laporan->jenis_kegiatan }}">{{ strtoupper($laporan->jenis_kegiatan) }}</span>
    ...
    <div class="section">
        ...
        <div class="info-grid">
            <div class="info-row">
                ...
                <div class="info-value">{{ $laporan->tanggal_pelaksanaan->format('d F Y') }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">Waktu</div>
                <div class="info-value">{{ $laporan->waktu_pelaksanaan }}</div>
            </div>
            <div class="info-row">
            ...
            <div class="info-row">
                ...
                <div class="info-value">{{ $laporan->penanggung_jawab }}</div>
            </div>
            @endif
            <div class="info-row">
                <div class="info-label">Jenis Kegiatan</div>
                <div class="info-value">{{ ucfirst($laporan->jenis_kegiatan) }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div class="stats-grid">
            <div class="stat-item">
                <div class="stat-value">{{ $laporan->jumlah_peserta }}</div>
                <div class="stat-label">Total Peserta</div>
            </div>
            <div class="stat-item">
                <div class="stat-value">{{ $laporan->jumlah_hadir }}</div>
                <div class="stat-label">Hadir</div>
            </div>
            <div class="stat-item">
                <div class="stat-value">{{ number_format($laporan->getPersentaseKehadiran(), 1) }}%</div>
    ...
    <div class="section">
        ...
        <p style="font-style: italic; color: #777;">
            Terdapat {{ count($laporan->foto_dokumentasi) }} foto dokumentasi yang tersimpan dalam sistem.
        </p>
    </div>
    @endif
        ...
        <table style="border: none;">
            <tr style="border: none;">
                <td style="border: none; text-align: left; width: 50%;">
                    Dibuat: {{ $laporan->created_at->format('d F Y, H:i') }} WIB
                </td>
