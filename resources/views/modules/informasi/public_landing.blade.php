    ...
    <nav class="bg-white shadow fixed w-full z-50 top-0">
        <div class="container mx-auto px-4 h-16 flex justify-between items-center">
            ...
            <div>
                ...
                @auth
                    <a href="{{ route('dashboard') }}" class="bg-green-700 text-white px-4 py-2 rounded hover:bg-green-800 ml-3">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="text-green-700 font-semibold ml-3">Login Pengurus</a>
                @endauth
            </div>
        </div>
    </nav>
    ...
    <div class="mt-16 bg-green-800 text-white py-20 text-center">
        ...
        <div id="jadwal" class="container mx-auto px-4">
            <div class="bg-white text-gray-800 rounded-lg shadow-xl p-6 max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div class="text-center p-2 rounded {{ $sholat == 'Maghrib' ? 'bg-green-100 border border-green-300' : 'bg-gray-50' }}">
            ...
            <div class="bg-white rounded-lg shadow hover:shadow-lg transition overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    <h3 class="text-lg font-bold mt-2 mb-2 text-gray-800 hover:text-green-700">
                        <a href="{{ route('public.info.show', $post->slug) }}">{{ $post->title }}</a>
                    </h3>
                    <p class="text-gray-600 text-sm line-clamp-3">{{ \Illuminate\Support\Str::limit(strip_tags($post->content), 100) }}</p>
                    <a href="{{ route('public.info.show', $post->slug) }}" class="inline-block mt-4 text-green-700 font-semibold text-sm">Baca Selengkapnya &rarr;</a>
                </div>
            </div>
            @empty
