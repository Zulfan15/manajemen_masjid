...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('takmir.aktivitas.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-6">
                ...
                <select name="takmir_id" id="takmir_id" required
                    ...
                    @foreach($pengurusList as $pengurus)
                        <option value="{{ $pengurus->id }}" {{ old('takmir_id') == $pengurus->id ? 'selected' : '' }}>
            ...
            <div class="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-blue-700">{{ $takmir->nama }} - {{ $takmir->jabatan }}</p>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="mb-6">
                ...
                <select name="jenis_aktivitas" id="jenis_aktivitas" required
                    ...
                    <option value="Ibadah" {{ old('jenis_aktivitas') == 'Ibadah' ? 'selected' : '' }}>Ibadah</option>
                    <option value="Kebersihan" {{ old('jenis_aktivitas') == 'Kebersihan' ? 'selected' : '' }}>Kebersihan</option>
                    <option value="Administrasi" {{ old('jenis_aktivitas') == 'Administrasi' ? 'selected' : '' }}>Administrasi</option>
                    <option value="Pengajaran" {{ old('jenis_aktivitas') == 'Pengajaran' ? 'selected' : '' }}>Pengajaran</option>
                    <option value="Pembinaan" {{ old('jenis_aktivitas') == 'Pembinaan' ? 'selected' : '' }}>Pembinaan</option>
                    <option value="Keuangan" {{ old('jenis_aktivitas') == 'Keuangan' ? 'selected' : '' }}>Keuangan</option>
                    <option value="Sosial" {{ old('jenis_aktivitas') == 'Sosial' ? 'selected' : '' }}>Sosial</option>
                    <option value="Lainnya" {{ old('jenis_aktivitas') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                </select>
            ...
            <div class="mb-6">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="5" required
                          ...
                          placeholder="Jelaskan secara detail aktivitas yang dilakukan...">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                <p class="text-gray-500 text-sm mt-1">
