...
@section('title', 'Pemilihan Ketua DKM')
...
<div class="container mx-auto px-4 py-8">
    ...
    <!-- Kandidat Cards -->
    <form method="POST" action="{{ route('takmir.pemilihan.submitVote', $pemilihan->id) }}" id="voteForm">
        @csrf
        <input type="hidden" name="kandidat_id" id="selectedKandidat" required>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            @foreach($pemilihan->kandidat as $kandidat)
            <div class="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition duration-300 kandidat-card" data-kandidat-id="{{ $kandidat->id }}">
                <!-- Nomor Urut Badge -->
                <div class="bg-gradient-to-r from-blue-600 to-blue-700 text-white text-center py-3">
                    ...
                    <div class="text-4xl font-bold">{{ $kandidat->nomor_urut }}</div>
                </div>
                ...
                <div class="p-6">
                    ...
                    <div class="text-center mb-4">
                        <h3 class="text-xl font-bold text-gray-800">{{ $kandidat->takmir->nama }}</h3>
                        <p class="text-gray-600 text-sm">{{ $kandidat->takmir->jabatan }}</p>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-sm text-gray-600 leading-relaxed">{{ $kandidat->visi }}</p>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="text-sm text-gray-600 leading-relaxed whitespace-pre-line">{{ $kandidat->misi }}</div>
                    </div>
                    ...
                    <button type="button" 
                            ...
                            data-kandidat-nama="{{ $kandidat->takmir->nama }}"
                            data-nomor-urut="{{ $kandidat->nomor_urut }}">
                        <i class="fas fa-check-circle mr-2"></i>
                        Pilih Kandidat Nomor {{ $kandidat->nomor_urut }}
                    </button>
                </div>
            </div>
            @endforeach
    ...
    <div class="text-center">
        <a href="{{ route('takmir.index') }}" class="inline-flex items-center text-gray-600 hover:text-gray-800">
