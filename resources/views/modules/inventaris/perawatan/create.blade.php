...
@section('content')
    <div class="container mx-auto px-4 max-w-4xl">
        ...
        <div class="bg-white rounded-xl shadow-sm p-8 border border-gray-100">
            <form action="{{ route('inventaris.perawatan.store') }}" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_jadwal" id="tanggal_jadwal" required
                            value="{{ old('tanggal_jadwal', date('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <select name="jenis_perawatan" id="jenis_perawatan" required
                            ...
                            @foreach($jenisPerawatan as $jenis)
                                <option value="{{ $jenis }}" {{ old('jenis_perawatan') == $jenis ? 'selected' : '' }}>
                                    {{ $jenis }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="dijadwalkan" {{ old('status', 'dijadwalkan') == 'dijadwalkan' ? 'selected' : '' }}>
                                Dijadwalkan</option>
                    ...
                    <div>
                        ...
                        <select name="id_petugas" id="id_petugas"
                            ...
                            @foreach($petugas as $user)
                                <option value="{{ $user->id }}" {{ old('id_petugas') == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="note" id="note" rows="4"
                            ...
                            placeholder="Catatan tambahan tentang perawatan...">{{ old('note') }}</textarea>
                        @error('note')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
