    ...
    <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg shadow p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold mt-2">Rp {{ number_format(\App\Models\Pengeluaran::sum('jumlah'), 0, ',', '.') }}</h3>
                    </div>
            ...
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg shadow p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold mt-2">{{ \App\Models\Pengeluaran::whereMonth('tanggal', date('m'))->whereYear('tanggal', date('Y'))->count() }}</h3>
                    </div>
            ...
            <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg shadow p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold mt-2">{{ \App\Models\KategoriPengeluaran::count() }}</h3>
                    </div>
        ...
        <div class="bg-gray-50 rounded-lg p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('keuangan.pengeluaran.index') }}" class="text-green-600 hover:text-green-700 text-sm font-medium">
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse(\App\Models\Pengeluaran::with('kategori')->latest('tanggal')->take(5)->get() as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ \Carbon\Carbon::parse($item->tanggal)->format('d/m/Y') }}
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900">
                                <div class="font-medium">{{ $item->judul_pengeluaran }}</div>
                                <div class="text-gray-500 text-xs">{{ \Illuminate\Support\Str::limit($item->deskripsi, 50) }}</div>
                            </td>
