...
@section('title', 'Pemasukan Masjid - CRUD Lengkap')
        ...
        <div class="col-xl-3 col-md-6">
            <div class="stat-card stat-card-success">
                ...
                <div class="stat-content">
                    ...
                    <h3 class="stat-value">Rp {{ number_format($data->sum('jumlah'), 0, ',', '.') }}</h3>
                    <div class="stat-badge">
                        <i class="fas fa-receipt"></i> {{ $data->count() }} Transaksi
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xl-3 col-md-6">
            <div class="stat-card stat-card-primary">
                ...
                <div class="stat-content">
                    ...
                    <h3 class="stat-value">
                        @if($data->count() > 0)
                            Rp {{ number_format($data->avg('jumlah'), 0, ',', '.') }}
                        @else
                            Rp 0
                        @endif
                    </h3>
        ...
        <div class="col-xl-3 col-md-6">
            <div class="stat-card stat-card-warning">
                ...
                <div class="stat-content">
                    ...
                    <h3 class="stat-value">Rp {{ number_format($bulanIni, 0, ',', '.') }}</h3>
                    <div class="stat-badge">
            ...
            <div class="table-container">
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        @endphp
                        @foreach($jenisData as $jenis => $config)
                            @php
                            ...
                            <tr>
                                <td>
                                    <span class="type-badge type-badge-{{ $config['color'] }}">
                                        <i class="fas {{ $config['icon'] }}"></i> {{ $jenis }}
                                    </span>
                                </td>
                    ...
                    <tfoot>
                        <tr class="table-primary">
                            ...
                            <th class="text-end">Rp {{ number_format($totalAll, 0, ',', '.') }}</th>
        ...
        <div class="data-card-header">
            ...
            <div class="data-card-actions">
                <div class="data-badge">
                    <i class="fas fa-database"></i> {{ $data->count() }} Data
                </div>
            </div>
        </div>
        <div class="data-card-body">
            <div class="table-container">
                <table class="custom-table" id="dataTable">
                    ...
                    <tbody>
                        ...
                        <tr data-tanggal="{{ $item->tanggal }}" data-id="{{ $item->id }}">
                            <td class="text-center">
                                <span class="row-number">{{ $loop->iteration }}</span>
                            </td>
                            <td>
                                <div class="date-info">
                                    <span class="date-primary">{{ \Carbon\Carbon::parse($item->tanggal)->translatedFormat('d F Y') }}</span>
                            ...
                            <td>
                                ...
                                @endphp
                                <span class="type-badge type-badge-{{ $config['color'] }}" data-jenis="{{ $item->jenis }}">
                                    <i class="fas {{ $config['icon'] }}"></i> {{ $item->jenis }}
                                </span>
                            </td>
                            <td>
                                @if($item->sumber)
                                    <div class="user-info">
                                        ...
                                        <span class="user-name" data-sumber="{{ $item->sumber }}">{{ $item->sumber }}</span>
                                    </div>
                                @else
                            ...
                            <td class="text-end">
                                <div class="amount-info">
                                    <span class="amount-primary" data-jumlah="{{ $item->jumlah }}">Rp {{ number_format($item->jumlah, 0, ',', '.') }}</span>
                                    @if($item->jumlah >= 1000)
                                        <small class="amount-secondary">{{ number_format($item->jumlah / 1000, 1) }}K</small>
                                    @endif
                                </div>
                            </td>
                            <td>
                                @if($item->keterangan)
                                    <div class="note-info">
                                        ...
                                        <span class="note-text" data-keterangan="{{ $item->keterangan }}" data-bs-toggle="tooltip" title="{{ $item->keterangan }}">
                                ...
                                <td>
                                    <div class="user-info">
                                        ...
                                        <div class="user-details">
                                            ...
                                            <small class="user-role">{{ $item->user->role ?? '—' }}</small>
                                        </div>
                                    </div>
                                </td>
                            @endif
                            <td class="text-center">
                                <div class="action-buttons">
                                    {{-- 🆕 BUTTON VERIFIKASI --}}
                                    @if(($item->status ?? 'pending') === 'pending' && auth()->user()->canManageKeuangan())
                                        <button type="button" 
                                                class="btn-action btn-action-success" 
                                                onclick="verifikasiTransaksi({{ $item->id }})"
                                                title="Verifikasi Transaksi">
                                            <i class="fas fa-check"></i>
                                        </button>
                                    @elseif(($item->status ?? 'pending') === 'verified')
                                        <span class="badge-verified" title="Sudah Terverifikasi pada {{ $item->verified_at ? $item->verified_at->format('d/m/Y H:i') : '-' }}">
                                            <i class="fas fa-check-circle"></i> Verified
                                        </span>
                                    @elseif(($item->status ?? 'pending') === 'rejected')
                                        <span class="badge-rejected" title="Ditolak: {{ $item->alasan_tolak ?? '-' }}">
                                    ...
                                    <button type="button" 
                                            ...
                                            onclick="editData({{ $item->id }}, '{{ $item->tanggal }}', '{{ $item->jenis }}', '{{ addslashes($item->sumber ?? '') }}', {{ $item->jumlah }}, '{{ addslashes($item->keterangan ?? '') }}')"
                                            title="Edit Data"
                                            @if(auth()->user()->role == 'jamaah') disabled @endif>
                                    ...
                                    {{-- 🆕 BUTTON TOLAK (Opsional) --}}
                                    @if(($item->status ?? 'pending') === 'pending' && auth()->user()->canManageKeuangan())
                                        <button type="button" 
                                                class="btn-action btn-action-dark" 
                                                onclick="tolakTransaksi({{ $item->id }})"
                        ...
                        @empty
                        <tr>
                            <td colspan="{{ auth()->user()->role == 'jamaah' ? 7 : 8 }}" class="text-center">
