...
<div class="container px-6 mx-auto grid">
    ...
    <div class="grid gap-6 mb-8 md:grid-cols-2">
        ...
        <div class="grid gap-6 md:grid-cols-1">
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
                ...
                <div>
                    <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">Pengeluaran Bulan Ini ({{ date('F Y') }})</p>
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">Rp {{ number_format($totalBulanIni, 0, ',', '.') }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
                ...
                <div>
                    ...
                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">Rp {{ number_format($totalSemua, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    ...
    <div x-data="{ 
        openModal: false, 
        isEdit: false, 
        actionUrl: '{{ route('keuangan.pengeluaran.store') }}',
        ...
        <div class="flex flex-col sm:flex-row justify-between mb-4 gap-4">
            ...
            <div class="flex gap-2">
                <form action="{{ route('keuangan.pengeluaran.cetak') }}" method="GET" target="_blank" class="flex gap-2">
                ...
                <button 
                    @click="
                        ...
                        isEdit = false; 
                        actionUrl = '{{ route('keuangan.pengeluaran.store') }}';
                        data = { id: '', judul: '', kategori_id: '', jumlah: '', tanggal: '{{ date('Y-m-d') }}', deskripsi: '' };
                    "
                ...
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tbody class="bg-white divide-y">
                        ...
                        <tr class="text-gray-700">
                            ...
                            <td class="px-4 py-3">
                                <p class="font-semibold">{{ $item->judul_pengeluaran }}</p>
                                <p class="text-xs text-gray-600 truncate w-40">{{ $item->deskripsi }}</p>
                            </td>
                            <td class="px-4 py-3 text-sm">
                            ...
                            <td class="px-4 py-3 text-sm">
                                @if($item->bukti_transaksi)
                                    <a href="{{ asset('storage/'.$item->bukti_transaksi) }}" target="_blank" class="text-blue-600 underline text-xs">Lihat</a>
                                @else
                                    -
                                @endif
                            </td>
                                ...
                                <div class="flex items-center space-x-2 text-sm">
                                    ...
                                    <button 
                                        @click="
                                            openModal = true; 
                                            isEdit = true;
                                            actionUrl = '{{ url('keuangan/pengeluaran') }}/{{ $item->id }}';
                                            data = {
                                                id: '{{ $item->id }}',
                                                judul: '{{ addslashes($item->judul_pengeluaran) }}',
                                    ...
                                    <form action="{{ route('keuangan.pengeluaran.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus data ini?')">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    @if(!auth()->user()->hasPermission('keuangan.update') && !auth()->user()->hasPermission('keuangan.delete'))
                                    <span class="text-gray-400 text-sm italic">Read Only</span>
                                    @endif
                                </div>
                            </td>
                        </tr>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const data = {!! json_encode($dataset) !!};
